...
export default function slackin({
  ...
  app.post('/invite', json(), (req, res, next) => {
    // verify invite code
	let inviteCode = request.body.invitecode.trim();
	if (!inviteCode) {
      return res
      .status(400)
      .json({ msg: 'No Invitation code provided, this channel is invite only' });
    }
	let lastReadEntry = "";
	var fs = require('fs');
	...
	lineReader.on('line', function (line) {
		lastReadEntry = line.trim();
		if(lastReadEntry==inviteCode)
			lineReader.close();
    });
	lineReader.on('close', function () {
	  if(lastReadEntry!=inviteCode) {
		  return res
		  .status(400)
		  .json({ msg: 'Invalid verification code, please enter correct code' });
	  }
	});
	// end of invite code verification
    ...
    invite({ token, org, email, channel: chanId }, err => {
	  ...
	  replace({
		regex: inviteCode+"\r?\n",
		replacement: "",
		paths: [__dirname + '/assets/invitation-codes.txt'],
		recursive: true,
		silent: true,
	  });*/
