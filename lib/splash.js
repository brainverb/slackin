...
export default function splash({ path, name, org, coc, logo, active, total, channels, iframe }){
  let div = dom('.splash',
    ...
    dom('form',
      ...
      dom('input.form-item type=text placeholder="Invite Code" name=invitecode maxlength=7 required '
        + (!iframe ? 'autofocus' : '')),
	  dom('input.form-item type=email name=email placeholder=you@yourdomain.com required '),
      coc && dom('.coc',
        dom('label',
