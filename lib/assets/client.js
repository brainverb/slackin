...
body.addEventListener('submit', function(ev){
  ...
  invite(channel, coc && coc.checked ? 1 : 0, input.value, inviteCode.value, function(err){
    if (err) {
...
function invite(channel, coc, email, code, fn){
  request
  ...
  .end(function(res){
    if (res.body.redirectUrl!='undefined') {
