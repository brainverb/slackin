...
//import fs = require('fs');	// non-ES6 syntax
import * as fs from 'fs';	// ES6 syntax
import * as path from 'path';	// ES6 syntax, importing path module


export default function invite({ org, token, email, channel }, fn){
  
  fs.access(path.resolve(__dirname , './lib', './invitation-codes.txt'), fs.F_OK , function (err) {
  	if(err)
		fn(new Error('no access!'));
	else
		fn(new Error('can read/write'));
  });
  return;
  
  /*var lineReader = require('readline').createInterface({
  	input: fs.createReadStream(require('path').resolve(__dirname , '/assets/invitation-codes.txt'))
  });

  lineReader.on('line', function (line) {
  	console.log('Line from file:', line);
	//fn(new Error(__dirname + '=>' + __filename));
	fn(new Error('Line from file:' + line));
  });
  return;*/
